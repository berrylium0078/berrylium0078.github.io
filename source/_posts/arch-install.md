...
仅记录 intel CPU 核显。其他请参见 [Driver installation - ArchWiki](https://wiki.archlinux.org/title/Xorg#Driver_installation)
...
不建议使用 noto-fonts-cjk，否则会出现<img src="/images/00/ill-chars.png" style="display: inline; vertical-align: middle;" width="7%">，[解决方法](https://wiki.archlinux.org/title/Localization/Simplified_Chinese#Chinese_characters_displayed_as_variant_(Japanese)_glyphs)
{% endnote %}
...
编辑 `/etc/environment`，添加一行 `XMODIFIERS=@im=fcitx5`，不然包括 VSCode 在内的部分应用无法使用输入法。

### 让我们播放声音！

[参考资料](https://bbs.archlinux.org/viewtopic.php?id=273969)
